...
class VideoAudioMixer(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # כותרת
        self.add_widget(Label(text="ELAZAR EDITOR V3.7", font_size='24sp', size_hint_y=0.1))
        
        # סייר קבצים - נתיב בטוח יותר להתחלה
        path = '/sdcard/Download' if platform == 'android' else os.path.expanduser("~")
        self.file_chooser = FileChooserIconView(path=path, filters=['*.mp4', '*.mp3'])
    ...
    def run_mix(self, instance):
        if not self.video_path or not self.audio_path:
            self.status_label.text = "Select both files first!"
            return

        output_path = "/sdcard/Download/mixed_output.mp4"
        self.status_label.text = "Mixing... Please wait"

        # וידוא הרשאות לקובץ ה-ffmpeg
        ...
        try:
            ...
            self.status_label.text = "Error: Build failed"

class ElazarEditorApp(App):
    def build(self):
        # בקשת הרשאות רק כשנכנסים לאפליקציה באנדרואיד
        if platform == 'android':
